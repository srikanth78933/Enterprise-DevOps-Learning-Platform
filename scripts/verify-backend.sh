#!/usr/bin/env bash
# Smoke-tests the backend through the Ingress. Used by backend/Jenkinsfile's
# "Verify" stage and scripts/helm-upgrade-backend.sh; safe to run manually.
set -euo pipefail
...
echo "==> Verifying backend health via ${LB_ADDRESS}"
