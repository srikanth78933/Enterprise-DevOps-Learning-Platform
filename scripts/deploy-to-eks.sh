...
# manual redeploy of the current backend/frontend images.
#
# Usage:
...
FRONTEND_IMAGE="${FRONTEND_IMAGE:-yourdockerhubuser/enterprise-devops-frontend}"
TAG="${1:-latest}"
...
kubectl set image deployment/frontend frontend="${FRONTEND_IMAGE}:${TAG}" -n "${NAMESPACE}"
